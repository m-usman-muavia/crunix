...
const BarrelSettings = () => {
  ...
  const fetchMarket = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/barrel/market`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to fetch barrel market data');
      }
  ...
  const fetchAdminStats = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/barrel/admin/stats`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('authToken')}`
        }
      });
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to fetch barrel stats');
      }

      setStats({
        total_barrel_purchased: Number(data.total_barrel_purchased || 0),
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/barrel/admin/price`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to update barrel price');
      }

      setSuccess('Barrel price updated successfully.');
      await fetchMarket();
  ...
  return (
    <div className="main-wrapper">
      <div className="main-container" style={{ paddingBottom: 20 }}>
        <div className="deposit-header">Barrel Admin</div>

        <div className="addplans-section" style={{ marginTop: 12 }}>
          <div className="addplans-card">
            <h3 className="addplans-header">Barrel Purchase Summary</h3>
            <p style={{ margin: '8px 0' }}>
              Total Barrel Purchased by Users: <strong>{Number(stats.total_barrel_purchased || 0).toFixed(6)} BARREL</strong>
            </p>
          </div>
        </div>
        ...
        <div className="addplans-section">
          <div className="addplans-card">
            <h3 className="addplans-header">Update Barrel Price</h3>

            {loading && <p>Loading barrel market data...</p>}
            {error && <p style={{ color: 'red' }}>{error}</p>}
            {success && <p style={{ color: 'green' }}>{success}</p>}

            <form className="addplans-form" onSubmit={handleSubmit}>
              <div className="input-group">
                <label>Price of 1 Barrel (AED)</label>
                <input
                  type="number"
                  min="0.01"
                  step="0.01"
        ...
        <div className="addplans-section" style={{ marginTop: 12 }}>
          <div className="addplans-card">
            <h3 className="addplans-header">Recent Barrel Price History</h3>
